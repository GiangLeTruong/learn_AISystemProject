...
const aiRecommendClient = require('../grpc_clients/aiRecommendClient');
...
service.recommenderTrainCart = async () => {
    try {
        ...
        const aiResponse = await aiRecommendClient.trainModel(trainingData);
...
service.recommenderPredictCart = async (userId) => {
    try {
        const aiResponse = await aiRecommendClient.predictRecommendation(userId, 5);
        ...
        const recommendedProductIds = aiResponse.recommended_product_ids;
